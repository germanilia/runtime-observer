...
Runtime Observer is not published to PyPI yet. Install the Python SDK and collector directly from this repository using GitHub `subdirectory` URLs. Browser projects can install the lightweight npm helper from the repository root.
...
Use custom spans for important internal operations:

```python
with observer.start_span("price quote", kind="function", attributes={"source": "cache"}):
    quote = calculate_quote()
```

## Browser ingestion

Install the browser helper directly from GitHub:
...
Because the browser endpoint accepts the API key in the URL, only use it from trusted/internal environments or put the collector behind a trusted proxy. CORS is open by default to support local dashboards and internal tools.
