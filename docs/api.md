...
The Python SDK and Node.js SDK `RUNTIME_OBSERVER_ENDPOINT` should be the collector base URL, for example `http://127.0.0.1:4319`; SDKs append the appropriate ingest path. Set `RUNTIME_OBSERVER_API_KEY` and `RUNTIME_OBSERVER_SERVICE_NAME` in each application. `RUNTIME_OBSERVER_PROJECT_NAME` is deprecated for SDKs: current collectors ignore it and use the project attached to the API key instead.
