...
def _payload(row: dict[str, Any]) -> dict[str, Any]:
    try:
        value = json.loads(row.get("payload_json") or row.get("sample_payload_json") or "{}")
    except json.JSONDecodeError:
        value = {}
...
def _dependency_label(kind: str, payload: dict[str, Any]) -> str:
    if kind == "db_query":
        tables = payload.get("tables") if isinstance(payload.get("tables"), list) else []
        target = payload.get("target") or payload.get("database") or payload.get("table") or (", ".join(tables) if tables else "database")
        operation = payload.get("operation") or payload.get("statement_fingerprint") or payload.get("statement_template") or "query"
        return f"DB {target} • {operation}"
    if kind == "http_client_call":
        return f"HTTP {payload.get('method') or 'GET'} {payload.get('host') or payload.get('url') or 'unknown'}"
    if kind == "llm_call":
        return f"LLM {payload.get('provider') or 'unknown'} {payload.get('model') or ''}".strip()
    return kind.replace("_", " ")


def build_trace_flow(traces: list[dict[str, Any]], events: list[dict[str, Any]], spans: list[dict[str, Any]], logs: list[dict[str, Any]], exceptions: list[dict[str, Any]]) -> dict[str, Any]:
    nodes: dict[str, dict[str, Any]] = {}
    edges: list[dict[str, str]] = []
    route_by_app: dict[str, str] = {}

    def add_node(node: dict[str, Any]) -> None:
        nodes[node["id"]] = {**nodes.get(node["id"], {}), **node}

    def add_edge(source: str, target: str, label: str = "called") -> None:
        if source != target and not any(edge["from"] == source and edge["to"] == target for edge in edges):
            edges.append({"from": source, "to": target, "label": label})

    for trace in traces:
        root_id = f"route:{trace.get('app_id')}:{trace.get('route_id') or trace.get('id')}"
        route_by_app[str(trace.get("app_id"))] = root_id
        add_node({
            "id": root_id,
            "type": "route",
            "label": f"{_first(trace.get('method'), 'HTTP')} {_first(trace.get('route_pattern'), 'unknown route')}",
            "service": trace.get("service_name"),
            "timestamp": trace.get("started_at"),
            "duration_ms": trace.get("duration_ms"),
            "status": trace.get("status_code"),
            "details": {"trace_id": trace.get("id"), "route_id": trace.get("route_id")},
        })
    if not route_by_app:
        root_id = "route:unknown"
        route_by_app[""] = root_id
        add_node({"id": root_id, "type": "route", "label": "Trace root", "service": "unknown", "details": {}})
    ...
    for span in spans:
        ...
        if not span_id:
            continue
        current = span_by_id.setdefault(str(span_id), dict(span))
        if span.get("duration_ms") is not None:
            current.update(span)
        current["started_at"] = min([v for v in [current.get("started_at"), span.get("started_at")] if v] or [None])
        current["finished_at"] = max([v for v in [current.get("finished_at"), span.get("finished_at")] if v] or [None])
    for span_id, span in span_by_id.items():
        node_id = f"span:{span_id}"
        add_node({
            "id": node_id,
            "type": "span",
            "label": _first(span.get("name"), _first(span.get("kind"), "span")),
            "service": span.get("service_name"),
            "timestamp": span.get("started_at") or span.get("finished_at"),
            "duration_ms": span.get("duration_ms"),
            "status": span.get("status"),
            "span_id": span_id,
            "parent_span_id": span.get("parent_span_id"),
            "details": _payload(span),
        })
    for span_id, span in span_by_id.items():
        parent = f"span:{span.get('parent_span_id')}" if span.get("parent_span_id") in span_by_id else route_by_app.get(str(span.get("app_id"))) or next(iter(route_by_app.values()))
        add_edge(parent, f"span:{span_id}", "calls")

    for event in events:
        kind = str(event.get("kind") or "")
        if kind not in {"db_query", "http_client_call", "llm_call", "exception_raised"}:
            continue
        payload = _payload(event)
        node_type = "exception" if kind == "exception_raised" else "dependency"
        node_id = f"event:{event.get('id') or event.get('event_id') or kind}:{len(nodes)}"
        label = payload.get("type") or _dependency_label(kind, payload)
        add_node({
            "id": node_id,
            "type": node_type,
            "kind": kind,
            ...
            "status": payload.get("status_code") or payload.get("status") or payload.get("error_type"),
            "span_id": event.get("span_id"),
            "details": payload,
        })
        parent = f"span:{event.get('span_id')}" if event.get("span_id") in span_by_id else route_by_app.get(str(event.get("app_id"))) or next(iter(route_by_app.values()))
        add_edge(parent, node_id, "raises" if node_type == "exception" else "calls")
    ...
    for log in logs:
        ...
        add_node({
            "id": node_id,
            "type": "log",
            "label": _first(log.get("message"), "log")[:180],
            "service": log.get("service_name"),
            "timestamp": log.get("timestamp"),
            "status": log.get("level"),
            "span_id": log.get("span_id"),
            "details": {"logger_name": log.get("logger_name"), "source_function": log.get("source_function")},
        })
        parent = f"span:{log.get('span_id')}" if log.get("span_id") in span_by_id else route_by_app.get(str(log.get("app_id"))) or next(iter(route_by_app.values()))
        add_edge(parent, node_id, "logs")

    # Aggregate exception rows are retained as fallback when raw exception events were not stored.
    if exceptions and not any(node.get("type") == "exception" for node in nodes.values()):
        for exc in exceptions:
            node_id = f"exception:{exc.get('id')}"
            add_node({"id": node_id, "type": "exception", "label": _first(exc.get("type"), "Exception"), "service": exc.get("service_name"), "timestamp": exc.get("last_seen"), "status": "ERROR", "details": {"message": exc.get("normalized_message")}})
            add_edge(route_by_app.get(str(exc.get("app_id"))) or next(iter(route_by_app.values())), node_id, "raises")

    ordered_nodes = sorted(nodes.values(), key=lambda node: (node.get("timestamp") or "", node.get("type") or ""))
...
def create_router() -> APIRouter:
    ...
    def trace_map(trace_id: str, db: Database = Depends(get_db)) -> dict[str, Any]:
        with db.connect() as conn:
            ...
            flow = build_trace_flow(traces, events, spans, flow_logs, exceptions)
            return {"trace_id": trace_id, "traces": traces, "events": events, "spans": spans, "logs": logs, "flow_logs": flow_logs, "nearby_background_logs": nearby_background_logs, "exceptions": exceptions, "dependencies": dependencies, "flow": flow, "timeline": timeline, "nearby_logs_all_apps": nearby_logs}
...
function eventPayload(item){try{return JSON.parse(item.payload_json||'{}')}catch{return {}}}function flowNodeHtml(n){const status=n.status?`<span class="pill">${esc(n.status)}</span>`:'';const ms=n.duration_ms!=null?`<span class="pill">${Math.round(Number(n.duration_ms))}ms</span>`:'';const detail=n.type==='dependency'&&n.details?esc(n.details.rendered_statement||n.details.statement_template||n.details.statement_fingerprint||n.details.operation||n.details.method||n.details.model||'').slice(0,220):'';return `<div class="flowNode ${esc(n.type)}"><span class="serviceBadge">${esc(n.service||'unknown service')}</span><div class="flowType">${esc(n.type)}${n.kind?' • '+esc(n.kind.replaceAll('_',' ')):''}</div><b>${esc(n.label||n.id)}</b>${detail?`<div class="small mono">${detail}</div>`:''}<div class="tabs" style="margin:8px 0 0">${status}${ms}${n.timestamp?`<span class="pill">${esc(n.timestamp)}</span>`:''}</div></div>`;}function renderMap(data){const flow=data.flow||{nodes:[],edges:[]};const byId=new Map((flow.nodes||[]).map(n=>[n.id,n]));const root=(flow.nodes||[]).find(n=>n.type==='route');const rows=(flow.edges||[]).map(e=>{const from=byId.get(e.from)||root;const to=byId.get(e.to);if(!to)return '';return `<div class="flowRow">${flowNodeHtml(from)}<div><div class="flowArrow"></div><div class="flowEdgeLabel">${esc(e.label||'calls')}</div></div>${flowNodeHtml(to)}</div>`;}).join('');const orphan=(flow.nodes||[]).filter(n=>!(flow.edges||[]).some(e=>e.to===n.id)&&n.type!=='route').map(n=>`<div class="flowRow"><div></div><div></div>${flowNodeHtml(n)}</div>`).join('');const rootHtml=root&&!rows?flowNodeHtml(root):'';return `<div class="mapCanvas"><div class="flowMap">${rows||rootHtml||'<div class="empty">No map nodes for this trace yet.</div>'}${orphan}</div></div>`;}
async function openTraceMap(traceId){selectedTraceId=traceId;renderTraceList();copyCache.delete(`trace:${traceId}`);const data=await api(`/api/traces/${encodeURIComponent(traceId)}/map`);const flowLogs=data.flow_logs||data.logs||[];const backgroundLogs=data.nearby_background_logs||[];const flow=data.flow||{nodes:[],edges:[]};$('drawerTitle').textContent='Request causal flow';$('drawerSub').textContent=`trace ${traceId} • ${flow.nodes.length} flow nodes • ${flow.edges.length} arrows • ${flowLogs.length} exact logs • ${backgroundLogs.length} nearby background logs hidden`;const logsHtml=flowLogs.map(l=>`<div class="log"><div class="logHeader"><span class="pill level-${esc(l.level)}">${esc(l.level||'LOG')}</span><span class="small">${esc(l.service_name)} • ${esc(l.timestamp)} • exact trace</span></div><div>${esc(l.message)}</div><div class="small mono">${esc(l.logger_name||'')} ${l.source_function?'• '+esc(l.source_function):''}</div></div>`).join('')||'<div class="empty">No logs directly correlated to this trace. Nearby background logs are intentionally separated below.</div>';const backgroundHtml=backgroundLogs.slice(0,80).map(l=>`<div class="log"><div class="logHeader"><span class="pill level-${esc(l.level)}">${esc(l.level||'LOG')}</span><span class="small">${esc(l.service_name)} • ${esc(l.timestamp)} • ${l.trace_id?'other trace':'background/no trace'}</span></div><div>${esc(l.message)}</div><div class="small mono">${esc(l.logger_name||'')} ${l.source_function?'• '+esc(l.source_function):''}</div></div>`).join('');const depRows=(data.dependencies||[]).map(d=>{const p=eventPayload(d);return `<tr><td>${esc(d.kind)}</td><td>${esc(p.target||p.host||(Array.isArray(p.tables)?p.tables.join(', '):''))}</td><td class="mono">${esc(p.rendered_statement||p.statement_template||p.statement_fingerprint||p.operation||p.method||p.request_body_preview||'').slice(0,420)}${p.parameters?`<div class="small mono">params: ${esc(String(p.parameters)).slice(0,240)}</div>`:''}</td><td>${p.duration_ms?Math.round(Number(p.duration_ms)):''}</td></tr>`}).join('');$('drawerBody').innerHTML=`<div class="tabs"><button class="primary" data-copy-trace="${esc(traceId)}">Copy full trace for AI</button><span id="copyStatus" class="copyOk small">Preparing copy context...</span><span class="pill">${data.traces.length} route</span><span class="pill">${data.spans.length} spans/functions</span><span class="pill">${data.dependencies.length} dependency calls</span><span class="pill">${flowLogs.length} exact logs</span><span class="pill">${flow.nodes.length} flow nodes</span><span class="pill">${backgroundLogs.length} nearby background hidden</span><span class="pill">${data.exceptions.length} errors</span></div><div class="explain">The causal flow is derived from trace_id plus span_id / parent_span_id, so each arrow means “this route or span produced that span, dependency, log, or exception.” Background work without the matching trace_id is separated below.</div>${renderMap(data)}<h2>Dependency details + inputs</h2><pre>${depRows?'<table><thead><tr><th>kind</th><th>target</th><th>operation/input</th><th>ms</th></tr></thead><tbody>'+depRows+'</tbody></table>':'No dependency calls captured for this request.'}</pre><h2>Exact flow logs</h2><div class="list">${logsHtml}</div><h2>Pretty-printed raw timeline</h2><pre>${esc(JSON.stringify((data.timeline||data.events).slice(0,160),null,2))}</pre><details><summary>Nearby background activity, not part of this trace (${backgroundLogs.length})</summary><div class="explain">These logs happened around the same time but have no matching trace_id. Use this only for environmental noise or scheduler investigations.</div><div class="list">${backgroundHtml||'<div class="empty">No nearby background logs.</div>'}</div></details>`;$('drawer').classList.add('open');prepareCopy(`trace:${traceId}`,`/api/traces/${encodeURIComponent(traceId)}/agent-context`,'Ready to copy trace context');}
