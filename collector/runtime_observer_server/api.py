...
def create_router() -> APIRouter:
    ...
    def trace_correlated_logs(
        trace_id: str,
        level: str | None = None,
        app_ids: str | None = None,
        same_project: bool = True,
        window_seconds: int = Query(180, ge=0, le=3600),
        limit: int = Query(500, ge=1, le=2000),
        db: Database = Depends(get_db),
    ) -> dict[str, Any]:
        selected_app_ids = set(parse_csv(app_ids))
        with db.connect() as conn:
            anchors = rows_to_dicts(
                conn.execute(
                    """
                    SELECT traces.*, apps.service_name, apps.project_name
            ...
            if not anchors:
                event_times = rows_to_dicts(
                    conn.execute(
                        """
                        SELECT events.app_id, events.timestamp started_at, events.timestamp finished_at, apps.service_name, apps.project_name
                        FROM events JOIN apps ON apps.id=events.app_id
                        WHERE events.trace_id=? ORDER BY events.timestamp LIMIT 50
                        """,
                        (trace_id,),
                    ).fetchall()
                )
            ...
            projects = {row.get("project_name") or "default" for row in anchors}
            anchor_app_ids = {row.get("app_id") for row in anchors if row.get("app_id")}
            times = [parse_ts(value) for row in anchors for value in (row.get("started_at"), row.get("finished_at")) if value]
            ...
            if same_project:
                ...
                base_where.append(f"COALESCE(apps.project_name,'default') IN ({placeholders})")
                params.extend(sorted(projects))
            if selected_app_ids:
                placeholders = ",".join("?" for _ in selected_app_ids)
                base_where.append(f"logs.app_id IN ({placeholders})")
                params.extend(sorted(selected_app_ids))
            if level:
                base_where.append("UPPER(logs.level)=UPPER(?)")
                params.append(level)

            exact_where = [*base_where, "logs.trace_id=?"]
            exact_params = [*params, trace_id, limit]
            exact_logs = rows_to_dicts(
                conn.execute(
                    f"""
                    SELECT logs.*, apps.service_name, apps.display_name, apps.project_name,
                      'exact_trace' correlation_type, 1 exact_trace_match
                    FROM logs JOIN apps ON apps.id=logs.app_id
            ...
            if start and end and window_seconds > 0:
                nearby_where = [*base_where, "logs.timestamp BETWEEN ? AND ?", "(logs.trace_id IS NULL OR logs.trace_id!=?)"]
                nearby_params = [*params, start, end, trace_id, limit]
                nearby_logs = rows_to_dicts(
                    conn.execute(
                        f"""
                        SELECT logs.*, apps.service_name, apps.display_name, apps.project_name,
                          'nearby_time' correlation_type, 0 exact_trace_match
                        FROM logs JOIN apps ON apps.id=logs.app_id
                        WHERE {' AND '.join(nearby_where)}
                        ORDER BY logs.timestamp LIMIT ?
                        """,
                        nearby_params,
                    ).fetchall()
            ...
            for log in [*exact_logs, *nearby_logs]:
                log["correlation"] = {
                    "type": log.get("correlation_type"),
                    "exact_trace_match": bool(log.get("exact_trace_match")),
                    "trace_id": trace_id,
            ...
            logs = sorted(logs_by_id.values(), key=lambda item: (item.get("timestamp") or "", item.get("service_name") or ""))[:limit]

            groups_by_app: dict[str, dict[str, Any]] = {}
            for log in logs:
                app_id = str(log.get("app_id"))
                group = groups_by_app.setdefault(
                    app_id,
                    {
                        "app_id": app_id,
                ...
                if log.get("exact_trace_match"):
                    group["exact_count"] += 1
                else:
                    group["nearby_count"] += 1
                group["logs"].append(log)

            return {
                "trace_id": trace_id,
                "anchor_app_ids": sorted(anchor_app_ids),
                "project_names": sorted(projects),
                "filters": {"level": level, "app_ids": sorted(selected_app_ids), "same_project": same_project, "window_seconds": window_seconds, "limit": limit},
                "correlation_window": {"start": start, "end": end, "seconds": window_seconds},
                "counts": {"exact": len(exact_logs), "nearby": len(nearby_logs), "returned": len(logs)},
                "logs": logs,
                "groups": sorted(groups_by_app.values(), key=lambda item: (item.get("project_name") or "", item.get("service_name") or "")),
            }
...
function correlatedLogCard(l){const c=l.correlation||{};const kind=c.exact_trace_match?'exact trace':'nearby';return `<div class="log"><div class="logHeader"><span class="pill level-${esc(l.level)}">${esc(l.level||'LOG')}</span><span class="small">${esc(l.timestamp)} • ${kind}</span></div><div>${esc(l.message)}</div><div class="small mono">${esc(l.logger_name||'')} ${l.trace_id?'• trace '+esc(l.trace_id):'• no trace'} ${c.window_seconds&&!c.exact_trace_match?'• ±'+esc(c.window_seconds)+'s':''}</div></div>`;}
function renderCorrelatedColumns(data){const groups=data.groups||[];const meta=data.filters||{};const cols=groups.map(g=>`<div class="appColumn"><div class="routeHeader"><b>${esc(g.display_name||g.service_name||g.app_id)}</b><span class="pill">${num((g.logs||[]).length)} logs</span></div><div class="small">${esc(g.project_name||'default')} • ${num(g.exact_count)} exact • ${num(g.nearby_count)} nearby</div><div class="list" style="margin-top:10px">${(g.logs||[]).map(correlatedLogCard).join('')||'<div class="empty">No logs for this app.</div>'}</div></div>`).join('');$('correlatedLogs').innerHTML=`<div class="explain">Correlated logs are scoped ${meta.same_project?'to the same project as the trace':'across all projects'} and grouped side-by-side by app/service. Exact trace-id matches are causal; nearby logs share only the time window.</div><div class="correlatedGrid">${cols||'<div class="empty">No logs match the selected filters.</div>'}</div>`;}
function traceLogFilterQuery(){const params=new URLSearchParams();const level=$('traceLogLevel')?.value||'';if(level)params.set('level',level);const apps=Array.from(document.querySelectorAll('[data-trace-app]:checked')).map(el=>el.value);if(apps.length)params.set('app_ids',apps.join(','));params.set('same_project',$('traceSameProject')?.checked?'true':'false');params.set('window_seconds',$('traceLogWindow')?.value||'180');params.set('limit','1000');return params.toString();}
async function refreshCorrelatedLogs(traceId){const data=await api(`/api/traces/${encodeURIComponent(traceId)}/correlated-logs?${traceLogFilterQuery()}`);renderCorrelatedColumns(data);$('correlatedCount').textContent=`${num(data.counts?.exact)} exact • ${num(data.counts?.nearby)} nearby`;}
async function openTraceMap(traceId){selectedTraceId=traceId;renderTraceList();copyCache.delete(`trace:${traceId}`);const [data,correlated]=await Promise.all([api(`/api/traces/${encodeURIComponent(traceId)}/map`),api(`/api/traces/${encodeURIComponent(traceId)}/correlated-logs?limit=1000`)]);const flowLogs=data.flow_logs||data.logs||[];const backgroundLogs=data.nearby_background_logs||[];$('drawerTitle').textContent='Triggered map';$('drawerSub').textContent=`trace ${traceId} • ${flowLogs.length} exact logs • ${data.events.length} events • ${data.dependencies.length} dependency calls • ${backgroundLogs.length} nearby background logs`;const depRows=(data.dependencies||[]).map(d=>{const p=eventPayload(d);return `<tr><td>${esc(d.kind)}</td><td>${esc(p.target||p.host||(Array.isArray(p.tables)?p.tables.join(', '):''))}</td><td class="mono">${esc(p.rendered_statement||p.statement_template||p.statement_fingerprint||p.operation||p.method||p.request_body_preview||'').slice(0,420)}${p.parameters?`<div class="small mono">params: ${esc(String(p.parameters)).slice(0,240)}</div>`:''}</td><td>${p.duration_ms?Math.round(Number(p.duration_ms)):''}</td></tr>`}).join('');const appOptions=(overview.apps||[]).filter(a=>!correlated.project_names?.length||correlated.project_names.includes(a.project_name||'default')).map(a=>`<label class="pill"><input type="checkbox" data-trace-app value="${esc(a.id)}"> ${esc(appName(a))}</label>`).join('');$('drawerBody').innerHTML=`<div class="tabs"><button class="primary" data-copy-trace="${esc(traceId)}">Copy full trace for AI</button><span id="copyStatus" class="copyOk small">Preparing copy context...</span><span class="pill">${data.traces.length} route</span><span class="pill">${data.spans.length} spans/functions</span><span class="pill">${data.dependencies.length} dependency calls</span><span class="pill" id="correlatedCount">${num(correlated.counts?.exact)} exact • ${num(correlated.counts?.nearby)} nearby</span><span class="pill">${data.exceptions.length} errors</span></div><div class="explain">The causal map uses exact trace-id events. The correlated log view below can include other apps in the same project, with exact-vs-nearby metadata kept explicit so unrelated background work is not confused with request flow.</div>${renderMap({...data,logs:flowLogs})}<h2>Dependency details + inputs</h2><pre>${depRows?'<table><thead><tr><th>kind</th><th>target</th><th>operation/input</th><th>ms</th></tr></thead><tbody>'+depRows+'</tbody></table>':'No dependency calls captured for this request.'}</pre><h2>Correlated flow logs</h2><div class="filterRow"><select id="traceLogLevel"><option value="">all levels</option><option>ERROR</option><option>WARNING</option><option>INFO</option><option>DEBUG</option></select><select id="traceLogWindow"><option value="30">±30s</option><option value="180" selected>±3m</option><option value="600">±10m</option></select><label class="pill"><input type="checkbox" id="traceSameProject" checked> same project only</label>${appOptions||'<span class="small">No apps loaded for selection</span>'}<button id="traceLogRefresh">Apply</button></div><div id="correlatedLogs"></div><h2>Raw causal timeline</h2><pre>${esc(JSON.stringify((data.timeline||data.events).slice(0,160),null,2))}</pre>`;$('drawer').classList.add('open');renderCorrelatedColumns(correlated);$('traceLogRefresh').onclick=()=>refreshCorrelatedLogs(traceId);$('traceLogLevel').onchange=()=>refreshCorrelatedLogs(traceId);$('traceLogWindow').onchange=()=>refreshCorrelatedLogs(traceId);$('traceSameProject').onchange=()=>refreshCorrelatedLogs(traceId);document.querySelectorAll('[data-trace-app]').forEach(el=>el.onchange=()=>refreshCorrelatedLogs(traceId));prepareCopy(`trace:${traceId}`,`/api/traces/${encodeURIComponent(traceId)}/agent-context`,'Ready to copy trace context');}
