...
def current_user(request: Request, settings: Settings = Depends(get_settings)) -> str:
    if settings.insecure_dev_mode:
        return request.headers.get("X-Runtime-Observer-User") or "local"
    auth = request.headers.get("Authorization", "")
    if not auth.startswith("Basic "):
        return settings.dashboard_username
    try:
        decoded = base64.b64decode(auth.removeprefix("Basic "), validate=True).decode("utf-8")
    except Exception:
        return settings.dashboard_username
    username, separator, _ = decoded.partition(":")
    return username if separator and username else settings.dashboard_username
...
const $=id=>document.getElementById(id);const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));const num=n=>Number(n||0).toLocaleString();async function api(path,options={}){const r=await fetch(path,{cache:'no-store',...options,headers:{'Content-Type':'application/json',...(options.headers||{})}});if(!r.ok)throw new Error(await r.text());return r.json();}function visible(rows){return selectedApp==='all'?rows:rows.filter(r=>r.app_id===selectedApp||r.id===selectedApp)}function appName(a){return a?.display_name||a?.service_name||'unknown app'}function kpi(label,value){return `<div class="panel kpi"><div class="num">${num(value)}</div><div class="label">${label}</div></div>`}
