...
<div class="layout"><aside class="stack"><section class="panel"><div class="panelTitle"><h2>Entry points</h2><span class="pill">click route</span></div><div id="entrypoints" class="content stack"></div></section></aside><main class="main"><section class="panel"><div class="panelTitle"><h2>Applications</h2><span class="pill" id="appCount">0</span></div><div id="appTabs" class="content tabs"></div></section><section id="kpis" class="kpis"></section><section class="grid"><div class="panel"><div class="panelTitle"><h2>Route performance</h2><div class="toolbar"><span class="pill" id="routePerfMeta">sortable</span><button id="restoreRoutesBtn" class="hideBtn" style="display:none">Restore hidden</button></div></div><div id="routes" class="content"></div></div><div class="panel"><div class="panelTitle"><h2>Activity summary</h2><span class="pill">live</span></div><div id="mix" class="content stack"></div></div></section><section class="panel"><div class="panelTitle"><h2 id="routeTitle">Select an entry point to inspect request traces</h2><span class="pill" id="routeMeta">no route selected</span></div><div class="content split"><div><div class="small" style="margin-bottom:8px">Requests for selected route</div><div id="traceList" class="list"></div></div><div><div class="small" style="margin-bottom:8px">Logs related to selected route/request</div><div id="routeLogs" class="list"></div></div></div></section><section class="grid"><div class="panel"><div class="panelTitle"><h2>Errors</h2><span class="pill">click for context</span></div><div id="errors" class="content list"></div></div><div class="panel"><div class="panelTitle"><h2>Dependencies</h2><div class="toolbar"><span class="pill">click for context</span><button id="restoreDepsBtn" class="hideBtn" style="display:none">Restore hidden</button></div></div><div class="content"><div class="explain">Dependencies are external systems or resources touched by the selected app. Use the type badge to distinguish DB, HTTP, packages, and LLM-style calls; target/operation explain what was called, metrics show volume and latency, and the latest sample links back to the triggering trace when available.</div></div><div id="deps" class="content stack"></div></div></section><section class="panel"><div class="panelTitle"><h2>Logs</h2><div class="toolbar"><button class="tabBtn active" data-logtab="all">All</button><button class="tabBtn" data-logtab="client">Client console</button><button class="tabBtn" data-logtab="backend">Backend</button><input id="logSearch" placeholder="Search log message"><select id="logWindow" title="How far back to show logs"><option value="5">last 5m</option><option value="15">last 15m</option><option value="60" selected>last 1h</option><option value="360">last 6h</option><option value="1440">last 24h</option><option value="0">all retained</option></select><select id="level"><option value="">all levels</option><option>ERROR</option><option>WARNING</option><option>INFO</option><option>DEBUG</option></select><button id="searchBtn">Search</button></div></div><div id="logs" class="content list"></div></section></main></div>
...
<script>
let overview={apps:[],totals:{},routes:[],dependencies:[],recent_logs:[],recent_errors:[],event_kinds:[],log_levels:[]};let entries=[];let selectedApp='all';let selectedRouteId=null;let selectedTraceId=null;let routeState=null;let isRefreshing=false;let logTab='all';let refreshTimer=null;let refreshMs=10000;let routeSort={key:'p95',dir:'desc'};let hiddenRoutes=new Set();let hiddenDeps=new Set();let prefsApiAvailable=true;let logWindowMinutes=Number(localStorage.getItem('runtimeObserverLogWindowMinutes')||60);const copyCache=new Map();
const $=id=>document.getElementById(id);const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));const num=n=>Number(n||0).toLocaleString();async function api(path){const r=await fetch(path,{cache:'no-store'});if(!r.ok)throw new Error(await r.text());return r.json();}function loadLocalPrefs(){try{return JSON.parse(localStorage.getItem('runtimeObserverPrefs')||'{}')}catch{return {}}}function applyPrefs(p){hiddenRoutes=new Set(p.hiddenRoutes||[]);hiddenDeps=new Set(p.hiddenDeps||[]);if(p.routeSort)routeSort=p.routeSort;}async function loadPrefs(){const local=loadLocalPrefs();applyPrefs(local);if(!prefsApiAvailable)return;try{const r=await fetch('/api/preferences/dashboard',{cache:'no-store'});if(!r.ok)throw new Error(String(r.status));const data=await r.json();applyPrefs({...local,...(data.preferences||data||{})});}catch{prefsApiAvailable=false;}}function savePrefs(){const prefs={hiddenRoutes:[...hiddenRoutes],hiddenDeps:[...hiddenDeps],routeSort};localStorage.setItem('runtimeObserverPrefs',JSON.stringify(prefs));if(prefsApiAvailable)fetch('/api/preferences/dashboard',{method:'PUT',headers:{'content-type':'application/json'},body:JSON.stringify(prefs)}).catch(()=>{prefsApiAvailable=false;});}function visible(rows){return selectedApp==='all'?rows:rows.filter(r=>r.app_id===selectedApp||r.id===selectedApp)}function appName(a){return a?.display_name||a?.service_name||'unknown app'}function kpi(label,value){return `<div class="panel kpi"><div class="num">${num(value)}</div><div class="label">${label}</div></div>`}
...
function routeKey(r){return `${r.app_id}|${r.method}|${r.route_pattern}`;}function depKey(d){return `${d.app_id}|${d.dependency_type}|${d.target}|${d.operation||''}`;}function sortValue(row,key){if(key==='route')return `${row.method||''} ${row.route_pattern||''}`.toLowerCase();if(key==='app')return (row.service_name||'').toLowerCase();if(key==='calls')return Number(row.call_count||0);if(key==='errors')return Number(row.error_count||0);if(key==='p50')return Number(row.p50_ms||0);if(key==='p95')return Number(row.p95_ms||0);if(key==='last_seen')return Date.parse(row.last_seen||'')||0;return 0;}function sortRoutes(rows){return [...rows].sort((a,b)=>{const av=sortValue(a,routeSort.key),bv=sortValue(b,routeSort.key);const cmp=typeof av==='string'?av.localeCompare(bv):av-bv;return routeSort.dir==='asc'?cmp:-cmp;});}
function renderRoutePerformance(rows){const hiddenRows=visible(overview.routes||[]).filter(r=>hiddenRoutes.has(routeKey(r)));const shown=sortRoutes(rows.filter(r=>!hiddenRoutes.has(routeKey(r))));$('routePerfMeta').textContent=`${shown.length} shown${hiddenRows.length?`, ${hiddenRows.length} hidden`:''}`;$('restoreRoutesBtn').style.display=hiddenRows.length?'inline-flex':'none';const table=shown.length?`<table class="dataTable"><thead><tr>${[['route','Route'],['app','App'],['calls','Calls'],['errors','Errors'],['p50','p50'],['p95','p95'],['last_seen','Last seen']].map(([k,l])=>`<th><button data-route-sort="${k}">${l}${routeSort.key===k?(routeSort.dir==='asc'?' ↑':' ↓'):''}</button></th>`).join('')}<th></th></tr></thead><tbody>${shown.slice(0,30).map(r=>`<tr><td><button class="hideBtn" data-open-route="${esc(r.id)}"><span class="mono">${esc(r.method)} ${esc(r.route_pattern)}</span></button></td><td>${esc(r.service_name)}</td><td>${num(r.call_count)}</td><td class="${r.error_count?'level-ERROR':''}">${num(r.error_count)}</td><td>${Math.round(Number(r.p50_ms||0))}ms</td><td><b>${Math.round(Number(r.p95_ms||0))}ms</b></td><td class="small">${esc(r.last_seen||'')}</td><td><button class="hideBtn" data-hide-route="${esc(routeKey(r))}">Hide</button></td></tr>`).join('')}</tbody></table>`:'<div class="empty">No visible routes. Restore hidden entries or exercise the app.</div>';const hiddenHtml=hiddenRows.length?`<details style="margin-top:12px"><summary class="small">Hidden routes (${hiddenRows.length})</summary><div class="stack" style="margin-top:8px">${hiddenRows.map(r=>`<div class="restoreRow"><span class="mono small">${esc(r.service_name)} • ${esc(r.method)} ${esc(r.route_pattern)}</span><button class="hideBtn" data-restore-route="${esc(routeKey(r))}">Restore</button></div>`).join('')}</div></details>`:'';$('routes').innerHTML=table+hiddenHtml;}
function depTarget(d){return d.target||'unknown target';}function depOperation(d){return d.operation||d.last_sample?.payload?.operation||d.last_sample?.payload?.method||d.last_sample?.payload?.statement_fingerprint||'unknown operation';}function depSample(d){const s=d.last_sample||{},p=s.payload||{};return p.route_pattern||p.source_function||p.source_file||s.trace_id||'no sample context';}
function renderDependencies(rows){const hiddenRows=visible(overview.dependencies||[]).filter(d=>hiddenDeps.has(depKey(d)));const shown=rows.filter(r=>!hiddenDeps.has(depKey(r)));$('restoreDepsBtn').style.display=hiddenRows.length?'inline-flex':'none';const cards=shown.length?shown.slice(0,12).map(r=>{const payload=JSON.stringify(r);const p95=r.p95_duration_ms?`${Math.round(Number(r.p95_duration_ms))}ms`:'';const avg=r.avg_duration_ms?`${Math.round(Number(r.avg_duration_ms))}ms`:'';return `<div class="depCard" data-dep="${esc(payload)}"><div class="routeHeader"><b>${esc(r.display)}</b><span class="pill typeBadge">${esc(r.dependency_type||'dep')}</span></div><div class="metricGrid"><div class="metric small">Target<b title="${esc(depTarget(r))}">${esc(depTarget(r)).slice(0,42)}</b></div><div class="metric small">Operation<b title="${esc(depOperation(r))}" class="mono">${esc(depOperation(r)).slice(0,42)}</b></div><div class="metric small">Calls / errors<b>${num(r.call_count)} / <span class="${r.error_count?'level-ERROR':''}">${num(r.error_count)}</span></b></div><div class="metric small">Avg / p95<b>${avg||'—'} / ${p95||'—'}</b></div></div><div class="routeHeader"><span class="small">Latest sample: ${esc(depSample(r))}</span><span>${r.last_sample?.trace_id?`<button class="hideBtn" data-open-trace="${esc(r.last_sample.trace_id)}">Open trace</button>`:''}<button class="hideBtn" data-hide-dep="${esc(depKey(r))}">Hide</button></span></div></div>`}).join(''):'<div class="empty">No visible dependency calls. Restore hidden entries or exercise the app.</div>';const hiddenHtml=hiddenRows.length?`<details style="margin-top:12px"><summary class="small">Hidden dependencies (${hiddenRows.length})</summary><div class="stack" style="margin-top:8px">${hiddenRows.map(d=>`<div class="restoreRow"><span class="small"><span class="pill typeBadge">${esc(d.dependency_type)}</span> ${esc(depTarget(d))} • ${esc(depOperation(d)).slice(0,80)}</span><button class="hideBtn" data-restore-dep="${esc(depKey(d))}">Restore</button></div>`).join('')}</div></details>`:'';$('deps').innerHTML=cards+hiddenHtml;document.querySelectorAll('.depCard').forEach(el=>el.onclick=e=>{if(e.target.closest('button'))return;openDependency(JSON.parse(el.dataset.dep));});}
...
$('refreshBtn').onclick=refresh;$('refreshInterval').onchange=e=>setRefreshIntervalMs(e.target.value);$('logWindow').onchange=e=>{logWindowMinutes=Number(e.target.value);localStorage.setItem('runtimeObserverLogWindowMinutes',String(logWindowMinutes));refresh();};syncLogWindowSelect();$('searchBtn').onclick=searchLogs;$('closeBtn').onclick=closeDrawer;$('restoreRoutesBtn').onclick=()=>{hiddenRoutes.clear();savePrefs();render();};$('restoreDepsBtn').onclick=()=>{hiddenDeps.clear();savePrefs();render();};document.querySelectorAll('[data-logtab]').forEach(b=>b.onclick=()=>{logTab=b.dataset.logtab;render();});document.addEventListener('click',ev=>{const sortBtn=ev.target.closest('[data-route-sort]');if(sortBtn){routeSort=routeSort.key===sortBtn.dataset.routeSort?{key:sortBtn.dataset.routeSort,dir:routeSort.dir==='asc'?'desc':'asc'}:{key:sortBtn.dataset.routeSort,dir:['route','app'].includes(sortBtn.dataset.routeSort)?'asc':'desc'};savePrefs();render();return;}const openRouteBtn=ev.target.closest('[data-open-route]');if(openRouteBtn){selectRoute(openRouteBtn.dataset.openRoute);return;}const hideRouteBtn=ev.target.closest('[data-hide-route]');if(hideRouteBtn){ev.preventDefault();ev.stopPropagation();hiddenRoutes.add(hideRouteBtn.dataset.hideRoute);savePrefs();render();return;}const restoreRouteBtn=ev.target.closest('[data-restore-route]');if(restoreRouteBtn){ev.preventDefault();ev.stopPropagation();hiddenRoutes.delete(restoreRouteBtn.dataset.restoreRoute);savePrefs();render();return;}const hideDepBtn=ev.target.closest('[data-hide-dep]');if(hideDepBtn){ev.preventDefault();ev.stopPropagation();hiddenDeps.add(hideDepBtn.dataset.hideDep);savePrefs();render();return;}const restoreDepBtn=ev.target.closest('[data-restore-dep]');if(restoreDepBtn){ev.preventDefault();ev.stopPropagation();hiddenDeps.delete(restoreDepBtn.dataset.restoreDep);savePrefs();render();return;}const depBtn=ev.target.closest('[data-copy-dep]');if(depBtn){ev.preventDefault();ev.stopPropagation();copyDependency(depBtn.dataset.copyDep,depBtn);return;}const traceBtn=ev.target.closest('[data-copy-trace]');if(traceBtn){ev.preventDefault();ev.stopPropagation();copyTrace(traceBtn.dataset.copyTrace,traceBtn);return;}const logBtn=ev.target.closest('[data-copy-log]');if(logBtn){ev.preventDefault();ev.stopPropagation();copyLog(logBtn.dataset.copyLog,logBtn);return;}const openTraceBtn=ev.target.closest('[data-open-trace]');if(openTraceBtn){ev.preventDefault();ev.stopPropagation();openTraceMap(openTraceBtn.dataset.openTrace);}});setRefreshIntervalMs($('refreshInterval').value);loadPrefs().finally(refresh);
</script></body></html>
'''
