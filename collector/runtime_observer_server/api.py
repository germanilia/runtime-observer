...
def create_router() -> APIRouter:
    ...
    def global_overview(log_window_minutes: int | None = Query(60), log_limit: int = Query(300, ge=20, le=2000), db: Database = Depends(get_db), settings: Settings = Depends(get_settings), user_id: str = Depends(current_user)) -> dict[str, Any]:
        ...
        with db.connect() as conn:
            ...
            by_kind = rows_to_dicts(conn.execute(f"SELECT events.app_id, apps.service_name, apps.display_name, events.kind, COUNT(*) count FROM events JOIN apps ON apps.id=events.app_id WHERE {visible_clause} GROUP BY events.app_id, apps.service_name, apps.display_name, events.kind ORDER BY count DESC", visible_params).fetchall())
            by_level = rows_to_dicts(conn.execute(f"SELECT logs.app_id, apps.service_name, apps.display_name, logs.level, COUNT(*) count FROM logs JOIN apps ON apps.id=logs.app_id WHERE {visible_clause} AND (logs.route_id IS NULL OR {log_route_clause}) GROUP BY logs.app_id, apps.service_name, apps.display_name, logs.level ORDER BY count DESC", [*visible_params, *log_route_params]).fetchall())
...
<script>
let overview={apps:[],totals:{},routes:[],dependencies:[],recent_logs:[],recent_errors:[],event_kinds:[],log_levels:[]};let entries=[];let hiddenPrefs=[];let showHidden=false;let selectedApp='all';let selectedRouteId=null;let selectedTraceId=null;let routeState=null;let isRefreshing=false;let logTab='all';let refreshTimer=null;let refreshMs=10000;let logWindowMinutes=Number(localStorage.getItem('runtimeObserverLogWindowMinutes')||60);const copyCache=new Map();
const $=id=>document.getElementById(id);const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));const num=n=>Number(n||0).toLocaleString();function showLogin(){if(refreshTimer)clearInterval(refreshTimer);$('loginOverlay').classList.remove('hidden');}function hideLogin(user){$('loginOverlay').classList.add('hidden');$('userText').textContent=user?`signed in as ${user.username}`:'';}async function api(path,options={}){const r=await fetch(path,{cache:'no-store',...options,headers:{'Content-Type':'application/json',...(options.headers||{})}});if(r.status===401){showLogin();throw new Error('Authentication required');}if(!r.ok)throw new Error(await r.text());return r.json();}async function checkAuth(){try{const data=await api('/api/auth/me');hideLogin(data.user);return true;}catch{return false;}}async function loginSubmit(ev){ev.preventDefault();$('loginError').textContent='';const r=await fetch('/api/auth/login',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({username:$('loginUsername').value,password:$('loginPassword').value})});if(!r.ok){$('loginError').textContent='Sign in failed';return;}const data=await r.json();hideLogin(data.user);setRefreshIntervalMs($('refreshInterval').value);refresh();}async function logout(){await fetch('/api/auth/logout',{method:'POST'});showLogin();}function visible(rows){return selectedApp==='all'?rows:rows.filter(r=>r.app_id===selectedApp||r.id===selectedApp)}function appName(a){return a?.display_name||a?.service_name||'unknown app'}function kpi(label,value){return `<div class="panel kpi"><div class="num">${num(value)}</div><div class="label">${label}</div></div>`}
...
function renderEntries(){const rows=visible(entries);$('entrypoints').innerHTML=rows.length?rows.map(r=>`<button class="entry ${selectedRouteId===r.id?'active':''}" data-route="${esc(r.id)}" style="${r.hidden?'opacity:.55':''}"><div class="routeHeader"><b>${esc(r.method)} ${esc(r.route_pattern)}</b><span class="pill">${r.hidden?'hidden • ':''}${num(r.call_count)} calls</span></div><div class="routeHeader"><span class="small">${esc(r.service_name)} • ${num(r.log_count)} logs • ${num(r.trace_count)} traces</span><button data-pref-action="${r.hidden?'restore':'hide'}" data-pref-kind="route" data-pref-id="${esc(r.id)}" data-pref-app="${esc(r.app_id)}">${r.hidden?'Restore':'Hide'}</button></div></button>`).join(''):'<div class="empty">No routes yet. Exercise the app and telemetry will appear here.</div>';$('hiddenPanel').style.display=showHidden?'block':'none';$('hiddenCount').textContent=hiddenPrefs.length;$('hiddenItems').innerHTML=hiddenPrefs.length?hiddenPrefs.map(p=>`<div class="entry"><b>${esc(p.target_kind)} ${esc(p.target_id)}</b><div class="routeHeader"><span class="small">hidden ${esc(p.updated_at)}</span><button data-pref-action="restore" data-pref-kind="${esc(p.target_kind)}" data-pref-id="${esc(p.target_id)}" data-pref-app="${esc(p.app_id||'')}">Restore</button></div></div>`).join(''):'<div class="empty">Nothing hidden.</div>';document.querySelectorAll('[data-route]').forEach(b=>b.onclick=()=>selectRoute(b.dataset.route));document.querySelectorAll('[data-pref-action]').forEach(b=>b.onclick=async ev=>{ev.preventDefault();ev.stopPropagation();await setHidden(b.dataset.prefKind,b.dataset.prefId,b.dataset.prefApp||null,b.dataset.prefAction==='hide');});$('showHiddenBtn').textContent=showHidden?'Hide hidden':'Show hidden';$('showHiddenBtn').classList.toggle('active',showHidden);}
...
function refreshLabel(){return refreshMs===0?'manual refresh':`live refresh every ${refreshMs/1000}s`;}function setRefreshIntervalMs(ms){refreshMs=Number(ms);if(refreshTimer)clearInterval(refreshTimer);refreshTimer=null;$('liveDot').classList.toggle('livePulse',refreshMs!==0);$('liveText').textContent=refreshLabel();if(refreshMs>0)refreshTimer=setInterval(refresh,refreshMs);}async function setHidden(kind,id,appId,hidden){if(hidden){await api('/api/preferences/hidden',{method:'POST',body:JSON.stringify({target_kind:kind,target_id:id,app_id:appId})});if(kind==='route'&&selectedRouteId===id){selectedRouteId=null;selectedTraceId=null;routeState=null;}}else{await api(`/api/preferences/hidden/${encodeURIComponent(kind)}/${encodeURIComponent(id)}${appId?'?app_id='+encodeURIComponent(appId):''}`,{method:'DELETE'});}await refresh();}async function refresh(){if(isRefreshing)return;isRefreshing=true;try{const [o,e,h]=await Promise.all([api(`/api/overview?${logWindowQuery()}`),api(`/api/entrypoints${showHidden?'?include_hidden=true':''}`),api('/api/preferences/hidden')]);overview=o;entries=e;hiddenPrefs=h;$('liveText').textContent=`${refreshLabel()} • logs ${logWindowMinutes?`last ${logWindowMinutes}m`:'all retained'} • updated ${new Date().toLocaleTimeString()}`;if(selectedRouteId)await loadRoute(selectedRouteId,false);render();syncLogWindowSelect();}catch(err){$('liveText').textContent='telemetry refresh failed';console.error(err);}finally{isRefreshing=false;}}
