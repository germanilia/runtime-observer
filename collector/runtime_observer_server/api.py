...
function fmtTs(value){if(!value)return '—';const d=new Date(value);if(!Number.isFinite(d.getTime()))return String(value);return d.toLocaleString(undefined,{year:'numeric',month:'short',day:'2-digit',hour:'2-digit',minute:'2-digit',second:'2-digit',timeZoneName:'short'});}function fmtShortTs(value){if(!value)return '—';const d=new Date(value);if(!Number.isFinite(d.getTime()))return String(value);return d.toLocaleTimeString(undefined,{hour:'2-digit',minute:'2-digit',second:'2-digit',timeZoneName:'short'});}function parseJsonMaybe(value){if(typeof value!=='string')return value;try{return JSON.parse(value)}catch{return value}}function prettyJson(value){return JSON.stringify(parseJsonMaybe(value)??{},null,2)}function jsonPre(value){return `<pre>${esc(prettyJson(value))}</pre>`}function kv(label,value){return `<div class="kv"><b>${esc(label)}</b><span>${esc(value??'—')}</span></div>`}
function renderApps(){$('appCount').textContent=overview.apps.length;$('appTabs').innerHTML=`<button class="tabBtn ${selectedApp==='all'?'active':''}" data-app="all">Overview: all apps</button>`+overview.apps.map(a=>`<button class="tabBtn ${selectedApp===a.id?'active':''}" data-app="${esc(a.id)}" title="Last seen ${esc(fmtTs(a.last_seen))}">${esc(appName(a))}</button>`).join('');document.querySelectorAll('[data-app]').forEach(b=>b.onclick=()=>{selectedApp=b.dataset.app;selectedRouteId=null;selectedTraceId=null;routeState=null;render();});}
function renderEntries(){const rows=visible(entries);$('entrypoints').innerHTML=rows.length?rows.map(r=>`<button class="entry ${selectedRouteId===r.id?'active':''}" data-route="${esc(r.id)}"><div class="routeHeader"><b>${esc(r.method)} ${esc(r.route_pattern)}</b><span class="pill">${num(r.call_count)} calls</span></div><span class="small">${esc(r.service_name)} • ${num(r.log_count)} logs • ${num(r.trace_count)} traces • last ${esc(fmtTs(r.last_seen))}</span></button>`).join(''):'<div class="empty">No routes yet. Exercise the app and telemetry will appear here.</div>';document.querySelectorAll('[data-route]').forEach(b=>b.onclick=()=>selectRoute(b.dataset.route));}
...
function refreshLabel(){return refreshMs===0?'manual refresh':`live refresh every ${refreshMs/1000}s`;}function setRefreshIntervalMs(ms){refreshMs=Number(ms);if(refreshTimer)clearInterval(refreshTimer);refreshTimer=null;$('liveDot').classList.toggle('livePulse',refreshMs!==0);$('liveText').textContent=refreshLabel();if(refreshMs>0)refreshTimer=setInterval(refresh,refreshMs);}async function refresh(){if(isRefreshing)return;isRefreshing=true;try{const [o,e]=await Promise.all([api(`/api/overview?${logWindowQuery()}`),api('/api/entrypoints')]);overview=o;entries=e;$('liveText').textContent=`${refreshLabel()} • logs ${logWindowMinutes?`last ${logWindowMinutes}m`:'all retained'} • updated ${fmtShortTs(new Date())}`;if(selectedRouteId)await loadRoute(selectedRouteId,false);render();syncLogWindowSelect();}catch(err){$('liveText').textContent='telemetry refresh failed';console.error(err);}finally{isRefreshing=false;}}
async function selectRoute(routeId){selectedRouteId=routeId;selectedTraceId=null;await loadRoute(routeId,true);render();}async function loadRoute(routeId,openFirst){routeState=await api(`/api/routes/${encodeURIComponent(routeId)}/requests`);const r=routeState.route;$('routeTitle').textContent=`${r.method} ${r.route_pattern}`;$('routeMeta').textContent=`${r.service_name} • ${num(r.call_count)} calls • p95 ${Math.round(Number(r.p95_ms||0))}ms • last ${fmtTs(r.last_seen)}`;if(openFirst&&routeState.traces?.length)selectedTraceId=routeState.traces[0].id;}
function closeDrawer(){$('drawer').classList.remove('open')}function copyTrace(traceId,btn=null){copyPrepared(`trace:${traceId}`,'✓ Copied full trace context for AI agent',btn,'Copy full trace for AI');}function showCopyStatus(message,isError=false){let el=$('copyStatus');if(!el){el=document.createElement('span');el.id='copyStatus';el.className='copyOk small';$('drawerBody')?.prepend(el);}el.textContent=message;el.style.display='inline-flex';el.style.marginLeft='8px';el.classList.toggle('level-ERROR',isError);el.classList.toggle('copyOk',!isError);if(!isError&&message.startsWith('✓'))setTimeout(()=>{el.textContent='';},3500);}function setCopyButton(btn,text,disabled=false){if(!btn)return;btn.textContent=text;btn.disabled=disabled;btn.style.opacity=disabled?'.7':'1';}function preparedTextarea(){let area=$('preparedCopyText');if(!area){area=document.createElement('textarea');area.id='preparedCopyText';area.setAttribute('aria-hidden','true');area.style.position='fixed';area.style.left='0';area.style.top='0';area.style.width='1px';area.style.height='1px';area.style.opacity='0.01';area.style.zIndex='-1';document.body.appendChild(area);}return area;}function writeClipboardTextNow(text){const area=preparedTextarea();area.value=text;area.focus();area.select();area.setSelectionRange(0,area.value.length);const ok=document.execCommand('copy');if(!ok)throw new Error('copy command failed');}function showManualCopy(text){$('drawerTitle').textContent='Manual copy';$('drawerSub').textContent='Automatic clipboard copy was blocked by the browser';let body=$('drawerBody');body.innerHTML='<div class="tabs"><span id="copyStatus" class="level-ERROR small">Browser blocked automatic copy. The full AI context is selected below. Press Cmd/Ctrl+C.</span></div><div class="explain">This is the same content the copy button tried to put on your clipboard.</div><textarea id="manualCopyText" style="width:100%;min-height:70vh;background:#050a12;color:var(--text);border:1px solid var(--line);border-radius:14px;padding:12px;font-family:ui-monospace,SFMono-Regular,Menlo,monospace"></textarea>';$('drawer').classList.add('open');const area=$('manualCopyText');area.value=text;area.focus();area.select();area.setSelectionRange(0,area.value.length);}function updateLogCopyButtons(logId){document.querySelectorAll(`[data-copy-log="${CSS.escape(logId)}"]`).forEach(btn=>{const state=logCopyState(logId);btn.disabled=state!=='ready'&&state!=='error';btn.textContent=logCopyLabel(logId);});}function prepareLogCopyBackground(logId){const key=`log:${logId}`;if(copyCache.has(key))return;copyCache.set(key,{status:'loading',text:''});api(`/api/logs/${encodeURIComponent(logId)}/agent-context`).then(data=>{copyCache.set(key,{status:'ready',text:data.text});updateLogCopyButtons(logId);}).catch(err=>{console.error(err);copyCache.set(key,{status:'error',text:''});updateLogCopyButtons(logId);});}function prepareCopy(key,path,readyText='Ready to copy'){copyCache.set(key,{status:'loading',text:''});api(path).then(data=>{copyCache.set(key,{status:'ready',text:data.text});preparedTextarea().value=data.text;showCopyStatus(readyText);}).catch(err=>{console.error(err);copyCache.set(key,{status:'error',text:''});showCopyStatus('Failed to prepare copy context',true);});}function copyPrepared(key,successMessage,btn,resetText){const item=copyCache.get(key);if(!item){showCopyStatus('Preparing copy context. Click again when ready.',true);setCopyButton(btn,'Preparing...',true);setTimeout(()=>setCopyButton(btn,resetText,false),900);return;}if(item.status==='loading'){showCopyStatus('Still preparing copy context — try again in a second',true);setCopyButton(btn,'Preparing...',true);setTimeout(()=>setCopyButton(btn,resetText,false),900);return;}if(item.status==='error'){showCopyStatus('Copy context failed to prepare',true);setCopyButton(btn,'Copy failed',false);return;}try{writeClipboardTextNow(item.text);showCopyStatus(successMessage);setCopyButton(btn,'✓ Copied',false);setTimeout(()=>setCopyButton(btn,resetText,false),2500);}catch(err){showManualCopy(item.text);setCopyButton(btn,'Manual copy opened',false);}}async function copyLog(logId,btn=null){const key=`log:${logId}`;const existing=copyCache.get(key);if(!existing||existing.status==='loading'){prepareLogCopyBackground(logId);setCopyButton(btn,'Preparing...',true);showCopyStatus('Still preparing log context. The button will enable automatically.',true);return;}if(existing.status==='error'){copyCache.delete(key);prepareLogCopyBackground(logId);setCopyButton(btn,'Preparing...',true);showCopyStatus('Retrying log context preparation...',true);return;}copyPrepared(key,'✓ Copied log context for AI agent',btn,'Copy for AI');}function copyDependency(depId,btn=null){copyPrepared(`dep:${depId}`,'✓ Copied dependency errors/context for AI agent',btn,'Copy dependency errors for AI');}
function dependencyKindLabel(kind){if(kind==='db_query'||kind==='db')return 'Database query';if(kind==='http_client_call'||kind==='http')return 'Outbound HTTP call';if(kind==='llm_call'||kind==='llm')return 'LLM call';return eventLabel(kind);}function eventPayload(item){try{return JSON.parse(item.payload_json||'{}')}catch{return {}}}function summarizePayload(kind,p){if(kind==='db_query'||kind==='db')return p.rendered_statement||p.statement_template||p.statement_fingerprint||p.operation||'database query';if(kind==='http_client_call'||kind==='http')return `${p.method||''} ${p.url||p.host||p.target||''} ${p.status_code?'→ '+p.status_code:''}`.trim();if(kind==='llm_call'||kind==='llm')return `${p.provider||'LLM'} ${p.model||''} ${p.input_tokens||p.output_tokens?`tokens in/out ${p.input_tokens||0}/${p.output_tokens||0}`:''}`.trim();return p.message||p.operation||p.name||'';}function dependencyDetailHtml(kind,p){return `<div class="keyGrid">${kv('Kind',dependencyKindLabel(kind))}${kv('Target',p.target||p.host||p.url||p.database||p.model||(Array.isArray(p.tables)?p.tables.join(', '):''))}${kv('Operation',p.operation||p.method||p.provider||p.statement_fingerprint)}${kv('Duration',p.duration_ms?Math.round(Number(p.duration_ms))+' ms':'—')}${kv('Status',p.status_code||p.status||p.error_type||'—')}${kv('Rows/tokens',p.row_count??(p.input_tokens||p.output_tokens?`${p.input_tokens||0} in / ${p.output_tokens||0} out`:'—'))}</div>${jsonPre(p)}`;}function openEventDetail(index){const item=drawerEventItems[index];if(!item)return;const p=item.kind==='log_record'?item:eventPayload(item);$('drawerTitle').textContent=item.kind==='log_record'?'Log record':dependencyKindLabel(item.kind);$('drawerSub').textContent=`${item.service_name||''} • ${fmtTs(item.timestamp)}`;$('drawerBody').innerHTML=`<div class="tabs">${item.trace_id?`<button data-open-trace="${esc(item.trace_id)}">Open trace</button>`:''}</div><div class="keyGrid">${kv('Event',eventLabel(item.kind))}${kv('Service',item.service_name)}${kv('Trace',item.trace_id||'none')}${kv('Time',fmtTs(item.timestamp))}</div>${item.kind==='log_record'?logDetailHtml(item):dependencyDetailHtml(item.kind,p)}`;$('drawer').classList.add('open');}
function logDetailHtml(log){return `<div class="keyGrid">${kv('Level',log.level||'LOG')}${kv('Logger',log.logger_name)}${kv('Source',log.source_function||log.source_file)}${kv('Route',log.route_pattern||log.route_id)}</div><div class="explain">${esc(log.message||'')}</div>${jsonPre(log)}`;}
async function openDependency(dep){const data=await api(`/api/dependencies/${encodeURIComponent(dep.id)}/context`);const sample=dep.last_sample||{};const samplePayload=sample.payload||{};const errorRows=(data.error_samples||[]).map(e=>`<div class="log"><div class="logHeader"><span class="pill level-ERROR">ERROR</span><span class="small">${esc(fmtTs(e.timestamp))} ${e.trace_id?'• trace '+esc(e.trace_id):'• no trace'}</span></div><div class="mono">${esc(e.payload?.error_type||e.payload?.error||'dependency error')}</div><div>${esc(e.payload?.error_message||'')}</div>${jsonPre(e.payload)}${e.trace_id?`<button data-open-trace="${esc(e.trace_id)}">Open trace</button>`:''}</div>`).join('');const related=(data.related_logs||[]).map(l=>`<div class="log"><div class="logHeader"><span class="pill level-${esc(l.level)}">${esc(l.level||'LOG')}</span><span class="small">${esc(l.service_name)} • ${esc(fmtTs(l.timestamp))}</span></div><div>${esc(l.message)}</div><div class="small mono">${esc(l.logger_name||'')}</div></div>`).join('');$('drawerTitle').textContent='Dependency context';$('drawerSub').textContent=`${dep.service_name} • ${num(dep.call_count)} calls • ${num(dep.error_count||0)} errors • latest ${fmtTs(sample.timestamp||dep.last_seen)}`;$('drawerBody').innerHTML=`<div class="explain">Aggregate dependency with a pretty-printed latest sample payload. Error samples are individual failed events behind the error count.</div><div class="tabs"><button class="primary" data-copy-dep="${esc(dep.id)}" style="pointer-events:auto;cursor:pointer">Copy dependency errors for AI</button><span id="copyStatus" class="copyOk small" style="min-width:220px">Preparing copy context...</span>${sample.trace_id?`<button data-open-trace="${esc(sample.trace_id)}">Open latest sample trace</button>`:''}</div><h2>Latest sample payload</h2>${dependencyDetailHtml(samplePayload.kind||sample.kind||dep.dependency_type,samplePayload)}<h2>Error samples</h2><div class="list">${errorRows||'<div class="empty">No individual error payloads retained for this dependency yet.</div>'}</div><h2>Related logs around error samples</h2><div class="list">${related||'<div class="empty">No nearby logs found.</div>'}</div><h2>Aggregate + context JSON</h2>${jsonPre(data)}`;$('drawer').classList.add('open');prepareCopy(`dep:${dep.id}`,`/api/dependencies/${encodeURIComponent(dep.id)}/agent-context`,'Ready to copy dependency context');}
async function openLog(log){$('drawerTitle').textContent='Log record';$('drawerSub').textContent=`${log.service_name} • ${fmtTs(log.timestamp)}`;$('drawerBody').innerHTML=`<div class="tabs"><button class="primary" data-copy-log="${esc(log.id)}">Copy log context for AI</button><span id="copyStatus" class="copyOk small">Preparing copy context...</span>${log.trace_id?`<button data-open-trace="${esc(log.trace_id)}">Open trace map</button>`:''}</div>${log.trace_id?'':'<div class="empty">This log has no trace id, so only the record and nearby logs are available.</div>'}${logDetailHtml(log)}`;$('drawer').classList.add('open');prepareCopy(`log:${log.id}`,`/api/logs/${encodeURIComponent(log.id)}/agent-context`,'Ready to copy log context');}
function renderMap(data){const traces=data.traces||[],deps=data.dependencies||[],exceptions=data.exceptions||[],logs=data.logs||[],spans=data.spans||[];const nodes=[];traces.forEach(t=>nodes.push(`<div class="nodeRow"><div class="node route"><b>HTTP route</b><div>${esc(t.service_name)}</div><div class="mono">${esc(t.method)} ${esc(t.route_pattern)}</div><div class="small">${Math.round(Number(t.duration_ms||0))}ms • status ${esc(t.status_code||'')} • ${esc(fmtTs(t.finished_at||t.started_at))}</div></div></div>`));spans.filter(s=>s.kind==='function').slice(0,12).forEach(s=>nodes.push(`<div class="nodeRow"><div class="arrow"></div><div class="node"><b>Function</b><div class="mono">${esc(s.name||'handler')}</div><div class="small">${Math.round(Number(s.duration_ms||0))}ms • ${esc(s.status||'')}</div></div></div>`));logs.slice(0,12).forEach(l=>nodes.push(`<div class="nodeRow"><div class="arrow"></div><div class="node"><b>${esc(l.source_function||l.logger_name||'log')}</b><div>${esc(l.message).slice(0,140)}</div><div class="small">${esc(l.service_name)} • ${esc(l.level||'LOG')} • ${esc(fmtShortTs(l.timestamp))}</div></div></div>`));deps.slice(0,20).forEach(d=>{const p=eventPayload(d);const tables=Array.isArray(p.tables)?p.tables.join(', '):'';const target=p.target||p.host||p.database||p.model||tables||'dependency';nodes.push(`<div class="nodeRow"><div class="arrow"></div><div class="node dep"><b>${dependencyKindLabel(d.kind)}</b><div>${esc(target)}</div><div class="small mono">${esc(summarizePayload(d.kind,p)).slice(0,360)}</div><div class="small">${p.duration_ms?Math.round(Number(p.duration_ms))+'ms':''}${p.row_count!=null?' • rows '+esc(p.row_count):''}${p.status_code?' • status '+esc(p.status_code):''}</div></div></div>`)});exceptions.forEach(e=>nodes.push(`<div class="nodeRow"><div class="arrow"></div><div class="node errorNode"><b>${esc(e.type)}</b><div>${esc(e.normalized_message)}</div><div class="small">${esc(fmtTs(e.last_seen))}</div></div></div>`));return `<div class="mapCanvas">${nodes.length?nodes.join(''):'<div class="empty">No map nodes for this trace yet.</div>'}</div>`;}
function renderTimeline(items){drawerEventItems=items.slice(0,160);return drawerEventItems.map((item,i)=>{const p=item.kind==='log_record'?item:eventPayload(item);const summary=item.kind==='log_record'?item.message:summarizePayload(item.kind,p);return `<button class="eventCard" data-event-detail="${i}"><div class="routeHeader"><b>${esc(eventLabel(item.kind))}</b><span class="pill">${esc(fmtTs(item.timestamp))}</span></div><div>${esc(summary||'No summary')}</div><div class="small mono">${esc(item.service_name||'')} ${item.trace_id?'• trace '+esc(item.trace_id):''}</div></button>`}).join('')||'<div class="empty">No timeline events captured.</div>';}
async function openTraceMap(traceId){selectedTraceId=traceId;renderTraceList();copyCache.delete(`trace:${traceId}`);const data=await api(`/api/traces/${encodeURIComponent(traceId)}/map`);const flowLogs=data.flow_logs||data.logs||[];const backgroundLogs=data.nearby_background_logs||[];const timeline=(data.timeline||data.events||[]);drawerEventItems=timeline.slice(0,160);$('drawerTitle').textContent='Triggered map';$('drawerSub').textContent=`trace ${traceId} • ${flowLogs.length} exact logs • ${timeline.length} timeline events • ${data.dependencies.length} dependency calls • ${backgroundLogs.length} nearby background logs hidden`;const logsHtml=flowLogs.map(l=>`<div class="log"><div class="logHeader"><span class="pill level-${esc(l.level)}">${esc(l.level||'LOG')}</span><span class="small">${esc(l.service_name)} • ${esc(fmtTs(l.timestamp))} • exact trace</span></div><div>${esc(l.message)}</div><div class="small mono">${esc(l.logger_name||'')} ${l.source_function?'• '+esc(l.source_function):''}</div></div>`).join('')||'<div class="empty">No logs directly correlated to this trace. Nearby background logs are intentionally separated below.</div>';const backgroundHtml=backgroundLogs.slice(0,80).map(l=>`<div class="log"><div class="logHeader"><span class="pill level-${esc(l.level)}">${esc(l.level||'LOG')}</span><span class="small">${esc(l.service_name)} • ${esc(fmtTs(l.timestamp))} • ${l.trace_id?'other trace':'background/no trace'}</span></div><div>${esc(l.message)}</div><div class="small mono">${esc(l.logger_name||'')} ${l.source_function?'• '+esc(l.source_function):''}</div></div>`).join('');const depRows=(data.dependencies||[]).map((d,i)=>{const p=eventPayload(d);return `<tr><td>${esc(dependencyKindLabel(d.kind))}</td><td>${esc(p.target||p.host||p.url||p.model||(Array.isArray(p.tables)?p.tables.join(', '):''))}</td><td class="mono">${esc(summarizePayload(d.kind,p)).slice(0,420)}${p.parameters?`<div class="small mono">params: ${esc(prettyJson(p.parameters)).slice(0,240)}</div>`:''}<div>${drawerEventItems.findIndex(x=>x.id===d.id)>=0?`<button data-event-detail="${drawerEventItems.findIndex(x=>x.id===d.id)}">Payload details</button>`:''}</div></td><td>${p.duration_ms?Math.round(Number(p.duration_ms)):''}</td></tr>`}).join('');$('drawerBody').innerHTML=`<div class="tabs"><button class="primary" data-copy-trace="${esc(traceId)}">Copy full trace for AI</button><span id="copyStatus" class="copyOk small">Preparing copy context...</span><span class="pill">${data.traces.length} route</span><span class="pill">${data.spans.length} spans/functions</span><span class="pill">${data.dependencies.length} dependency calls</span><span class="pill">${flowLogs.length} exact logs</span><span class="pill">${backgroundLogs.length} nearby background hidden</span><span class="pill">${data.exceptions.length} errors</span></div><div class="explain">Only exact trace-id events are shown in the causal flow. Click timeline cards or dependency payload buttons for pretty-printed log/db/http/llm payload details.</div>${renderMap({...data,logs:flowLogs})}<h2>Dependency details + inputs</h2><div>${depRows?'<table><thead><tr><th>kind</th><th>target</th><th>operation/input</th><th>ms</th></tr></thead><tbody>'+depRows+'</tbody></table>':'<div class="empty">No dependency calls captured for this request.</div>'}</div><h2>Exact flow logs</h2><div class="list">${logsHtml}</div><h2>Causal timeline</h2><div class="list">${renderTimeline(timeline)}</div><details><summary>Pretty JSON timeline (${timeline.length})</summary>${jsonPre(timeline.slice(0,160))}</details><details><summary>Nearby background activity, not part of this trace (${backgroundLogs.length})</summary><div class="explain">These logs happened around the same time but have no matching trace_id. Use this only for environmental noise or scheduler investigations.</div><div class="list">${backgroundHtml||'<div class="empty">No nearby background logs.</div>'}</div></details>`;$('drawer').classList.add('open');prepareCopy(`trace:${traceId}`,`/api/traces/${encodeURIComponent(traceId)}/agent-context`,'Ready to copy trace context');}
async function openError(appId,id){const data=await api(`/api/apps/${encodeURIComponent(appId)}/exceptions/${encodeURIComponent(id)}`);if(data.exception?.sample_trace_id){await openTraceMap(data.exception.sample_trace_id);return;}$('drawerTitle').textContent='Error';$('drawerSub').textContent=fmtTs(data.exception?.last_seen);$('drawerBody').innerHTML=`${jsonPre(data)}`;$('drawer').classList.add('open');}
